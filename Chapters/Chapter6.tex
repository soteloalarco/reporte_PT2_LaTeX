...
\begin{algorithm}
    ...
    \For{$i$ in range(1,8)} \newline
    // se crean los dispositivos especificados para cada servicio \newline
    $D$.append( crearDispositivos($S_i$,$R_{célula}$)) \newline
    // se crea un generador de alarma para cada servicio\newline
    $G$.append( crearGeneradorAlarmas($S_i$,$R_{célula}$)) \newline
    \textbf{end for}
    \EndFor \\
$tiempo=0$
...
// rutina principal
    \While{$tiempo < T_{límite}$}{}
        \For{$dispPorServicio$, $generadorAlarma$ in $D$, $G$}
            ...
            \If{$tiempo==0$}  \newline
            // se crea la primera alarma para cada servicio \newline
            $nuevaAlarma$ = $generadorAlarma$.generarAlarma($tiempo$) \newline
            \textbf{end if}
            \EndIf

            \For{dispositivo in dispPorServicio} \newline
            // ciclo que recorre cada dispositivo de un servicio\newline
            \If{$dispositivo$.modeloTrafico == CMMPP}\newline
            $dispositivo$.registrarAlarma(nuevaAlarma)\newline
            $dispositivo$.calcularPnk($tiempo$,generadorAlarma)\newline
            $dispositivo$.actualizarEstado()\newline
            $dispositivo$.generarPaquete($tiempo$)\newline
            \Else \newline
            $dispositivo$.generarPaquetePeriodico($tiempo$)\newline
