...
Es importante notar que los dispositivos uRLLC tienen requerimientos de tasas de datos más altos, por lo tanto, su potencia de transmisión será mayor que la de los dispositivos mMTC. Es por esto que en cada grupo los dispositivos uRLLC tendrán los rangos más bajos y los dispositivos mMTC los más altos. De hecho, el decodificador SIC en la BS comienza a decodificar con URLLC y, en consecuencia, los dispositivos mMTC no se ven afectados por la alta interferencia de los URLLC, en la Figura~ref{NOMAgrupoexample} se puede observar un ejemplo de un agrupamiento de de 4 usuarios uRLLC y 8 usuarios mMTC en 3 grupos NOMA, es importante notar que si los dispositivos URLLC no se alcanzan a agrupar en el primer rango de los grupos NOMA, estos se seguirán agrupando hasta un segundo rango.\newline
...
Este algoritmo garantiza una óptima asignación de portadoras (S), de acuerdo con la maximización de la tasa total de transmisión, para los grupos NOMA parencite{Shahini2019}. En el \textit{Algoritmo \ref{A2}} se plantea la metodología para la asignación de subportadoras a los grupos NOMA. \newline

Después de haberse creado los grupos NOMA con el Algoritmo ref{A1}, se procede a asignar los recursos (subportadoras), para cada subportadora, el mejor grupo $c^*$ es el que maximiza el rendimiento total. Luego, por consecuencia, las velocidades de datos de los dispositivos mMTC y URLLC y sus potencias de transmisión se actualizan. Durante el proceso de asignación de subportadoras, los clústeres asignados se excluyen del conjunto de Cns. \newline
...
La fase de asignación de recursos termina hasta que las 48 subportadoras se asignan a los 48 clústeres NOMA, es decir una subportadora por grupo, esto del hecho que se adaptó el algoritmo para una transmisión singletone. \newline
...
\begin{algorithm}
    ...
    En este escenario de pruebas, se evaluó el rendimiento del sistema del esquema NB-IoT basado en NOMA propuesto en \parencite{Shahini2019}, se programaron en python los dos Algoritmos, el de formación de grupos y el de asignación de recursos. \newline
    
    Su modelo de sistema consideró una celda con un radio de 0.5 km donde las ubicaciones de los dispositivos mMTC y URLLC se generaron aleatoriamente (de manera uniforme) dentro de la celda. Además, consideraron un PRB con 48 subportadoras de 3.75 kHz para las transmisiones de enlace ascendente (UL) en un solo intervalo de tiempo (TTI). \newline
    
    Modelaron las ganancias de canal de los dispositivos mMTC como $h_{s}^{m} = \gamma d^{ -\beta}_{ m, s}$ (de manera similar $h_{s}^{u}$ para los URLLCs), donde $\gamma$ es un valor aleatorio generado por la PDF de Rayleigh, $d_{-\beta}^{m,s}$ representa la distancia entre el UE y la BS, y $\beta$ es el PLE el cual lo establecieron en $\beta = 3$ y $d$ varía entre 0.1 y 500 m. Se generó el histograma (pdf) de los coeficientes de canal (pérdidas del canal) esperados para el este modelo de sistema, véase Figura~\ref{fig: CoeficientesCanalShahini}, se puede observar que el valor promedio de las ganancias ($h$) es de 74.67 dB. \newline
    ...
    Cabe destacar que los autores en \parencite{Shahini2019} propusieron la oportunidad de asignar más de una subportadora (\textit{multitone}) por grupo NOMA en transmisiones UL con anchos de banda de 3.75KHz en NB-IoT. Entonces, aunque aún no ha sido especificada esta característica, se tomó la propuesta de la asignación de múltiples subportadoras por cluster en anchos de banda de 3.75KHz en este simulador. \newline
    
    Es importante enfatizar que no necesariamente deben coincidir nuestros resultados con los del artículo ya que se tomaron algunas consideraciones diferentes a la de ellos. En la implementación del Algoritmo \ref{A2} se percató que los autores \parencite{Shahini2019} siempre buscan alcanzar la tasa objetivo de los dispositivos uRLLC y mMTC en cada grupo, realizando la agregación de multiples subportadoras a cada grupo, el problema que surge con esto es que muchas de las veces que se corría la simulación, la mayoría de las subportadoras se asignaban de entre 1 - 6 clusters esto porque parecía difícil cumplir con las tasas de todos los dispositivos en estos grupos, dejando a los otros grupos sin subportadora. La modificación que se implementó es que siempre se logre asignar una subportadora para cada grupo aunque sus dispositivos no alcancen su tasa objetivo, es decir se adoptó un número fijo de grupos NOMA igual a 48. \newline
    
    Por otra parte, se intentó aplicar una transmisión \textit{multitone} de otro modo (esto con fines de una evaluación comparativa con singletone), analizemos el siguiente ejemplo: \newline
    
    Si en un TTI, 50 dispositivos desean transmitir, en un modo \textit{singletone} se crearían siempre 48 grupos, entonces, 48 usuarios se agruparían en el primer rango de cada grupo y los otros dos restantes se agruparían en el segundo rango de los dos primeros grupos. En este caso no puede hacerse \textit{multitone} ya que las 48 subportadoras serían ocupadas por cada grupo NOMA. \newline
    
    Ahora bien, un modo \textit{multitone} se podría aplicar de la siguiente manera: si se consideran 50 dispositivos queriendo transmitir, estos se pueden dividir en dos grupos de 25 dispositivos, es decir crear 25 grupos con su primer y segundo rangos completos, esto con la finalidad de que las 23 subportadoras restantes se agreguen en los grupos donde sus dispositivos aún no alcanzan su tasa objetivo, esto se implementó siguiendo la metodología del $Algorithm$ \ref{A3}. \newline
    
    En la Figura \ref{fig:ResultsNOMA} se muestra una gráfica de la evaluación del número de usuarios que alcanzan su tasa objetivo en un TTI, se compararon los resultados de NOMA del artículo, los resultados NOMA de nuestra simulación (en modo singletone y multitone) y OFDMA. Se puede observar que los resultados del artículo muestran mejor desempeño cuando el núm. de usuarios ronda entre los 50 y 175 usuarios, y cuando llegan a ser 192 usuarios el rendimiento de ambos es muy similar. También se agregó el desempeño de OFDMA donde solo se puede dar servicio a un usuario por subportadora, lo que son 48 usuarios.\newline
    
    Con respecto a los resultados entre \textit{multitone} y \textit{singletone} de nuestra simulacion, se puede ver que el desempeño de ambos modos de operación es muy similar en varios puntos. EN particular, en multitone, cuando son 25 usuarios , este logra un mayor rendimiento a comparación que singletone. Esto se debe a que en singletone cuando el número de dispositivos es menor al número de clusters, hay desperdicio de recursos, por ej. con 25 grupos, estos se agrupan a solo 25 subportadoras dejando a las restantes sin utilizar, caso contrario con multitone, donde sí se aprovecharían todas.\newline
    
    En la figura \ref{fig:ResultsNOMA} se muestra una gráfica de la evaluación de la tasa suma alcanzable en un TTI, se compararon los resultados de NOMA del artículo y los resultados NOMA de nuestra simulación en modo singletone y multitone. Se puede observar que \textit{multitone} alcanza una mayor tasa-suma en comparación con \textit{singletone}, esto es por lo que se comentó anteriormente, que \textit{multitone} no desperdicia subportadoras por eso alcanza una mayor tasa-suma. Pero que llega a empatar (con\textit{singletone}) cuando son 48 usuarios, esto es porque ya no hay más subportadoras que se puedan agregar.
    ...
    El impacto de la agregación de las subportadoras solo beneficia a aquellos dispositivos que tienen una buena condición de canal o que están en los últimos rangos (ya que experimentan menos interferencia), por lo tanto, la compensación de tasa no es equitativa para todos los dispositivos en un grupo NOMA. \newline
